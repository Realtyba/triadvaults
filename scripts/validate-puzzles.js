...
function build(Type, playersCount, level = 20, seed = 7919) {
  ...
  puzzle.generate(info, playersCount);
  return puzzle;
}
...
for (const Type of PUZZLE_TYPES) {
  ...
  const puzzle = build(Type, playersCount);
...
const sequence = build(Sequence, 1);
...
console.log('');
console.log(failures === 0 ? '✓ Todos los arquetipos son jugables.' : `✗ ${failures} comprobación(es) fallida(s).`);
